 ...
 * - Keys et al. (1972): Indices of relative weight and obesity. Journal of Chronic Diseases
 * - BMI Formula: Quetelet Index (1832)
 *
 ...
 * - Obese: BMI ≥ 30
 *
 * Children: Uses CDC growth chart percentiles
 */
...
export function getBMIPercentileCategory(percentile: number): { name: string; color: string } {
  ...
  const category = BMI_PERCENTILE_CATEGORIES.find(
    cat =>
      percentile >= cat.range.min && (cat.range.max === undefined || percentile < cat.range.max)
  );
...
export function processBMICalculation(values: BMIFormValues): BMIResult {
  ...
  const heightCm =
    values.heightUnit === 'cm' ? values.heightCm : values.heightFt * 30.48 + values.heightIn * 2.54;

  // Convert weight to kg if needed
  const weightKg = values.weightUnit === 'kg' ? values.weightKg : values.weightLb * 0.453592;
