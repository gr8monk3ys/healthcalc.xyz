...
export function calculateNavyMethodBodyFat(
  ...
  if (gender === 'male') {
    ...
    bodyFatPercentage = 495 / (1.0324 - 0.19077 * Math.log10(waistIn - neckIn) + 0.15456 * Math.log10(heightIn)) - 450;
  } else {
    ...
    bodyFatPercentage = 495 / (1.29579 - 0.35004 * Math.log10(waistIn + hipsIn - neckIn) + 0.22100 * Math.log10(heightIn)) - 450;
  }
 ...
 * @returns Body fat percentage
 */
export function calculateBMIMethodBodyFat(
  gender: Gender,
  age: number,
  bmi: number
): number {
 ...
 */
export function getBodyFatCategory(gender: Gender, bodyFatPercentage: number): { name: string; color: string } {
...
export function calculateBodyFat(
  ...
  const { gender, age, heightCm, weightKg, waistCm, neckCm, hipsCm, bmi, bodyFatPercentage } = params;
