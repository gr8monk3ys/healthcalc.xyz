...
interface CaloriesBurnedCyclingResultProps {
  result: CaloriesBurnedCyclingResult | null;
}

export default function CaloriesBurnedCyclingResult({ result }: CaloriesBurnedCyclingResultProps) {
  if (!result) {
    return (
      <div className="neumorph p-6 rounded-lg">
        <h2 className="text-xl font-semibold mb-2">Calories Burned Cycling Result</h2>
        <p className="text-gray-600">Enter your cycling details to see results.</p>
      </div>
    );
  }

  return (
    <div id="calories-burned-cycling-result" className="neumorph p-6 rounded-lg animate-fade-in">
      <h2 className="text-xl font-semibold mb-4">Calories Burned Cycling</h2>
      <div className="neumorph-inset p-5 rounded-lg mb-4">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
          Speed: {result.speedMph} mph • {result.durationMinutes} minutes
        </p>
      </div>
