...
interface CaloriesBurnedRunningResultProps {
  result: CaloriesBurnedRunningResult | null;
}

export default function CaloriesBurnedRunningResult({ result }: CaloriesBurnedRunningResultProps) {
  if (!result) {
    return (
      <div className="neumorph p-6 rounded-lg">
        <h2 className="text-xl font-semibold mb-2">Calories Burned Running Result</h2>
        <p className="text-gray-600">Enter your run details to see results.</p>
      </div>
    );
  }

  return (
    <div id="calories-burned-running-result" className="neumorph p-6 rounded-lg animate-fade-in">
      <h2 className="text-xl font-semibold mb-4">Calories Burned Running</h2>
